...
# sync package when stripe key changes
@receiver(post_save, sender=SubscriptionPackage)
def create_stripe_product(sender, instance, created, **kwargs):
    ...
    if not instance.stripe_product_id or not instance.stripe_price_id:
        ...
        try:
            ...
            SubscriptionPackage.objects.filter(pk=instance.pk).update(
                stripe_product_id=stripe_product_id,
                stripe_price_id=stripe_price_id,
            )
        except stripe.error.StripeError:
            logger.exception("Stripe sync failed for SubscriptionPackage id=%s", instance.pk)
